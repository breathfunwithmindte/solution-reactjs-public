...
export default function SimpleForm({ sector, reload, initialState }) {
  const { state } = useMain();
  const { search } = useLocation();
  const [formstate, setFormstate] = useState(() => initialState || new Object());
  ...
  const [actions, setActions] = useState(pr => {
    const defaultactions = [
      {
        label: "Submit", action: "primary-submit", when: "simpleform",
        script: null, script_onresponse: '.', classname: "w-350", 
        method: "POST", url: FormApiTypes.ApiTypes.CREATE_ONE(sector, { user: state.user }) + "?form=simpleform"
      }
    ]
    if(sector.actions instanceof Array === false) return defaultactions;
    const filtered_actions = sector.actions.filter(f => ["simpleform", "always"].some(s => s === f["when"]));
  ...
  const [formrenderstate, setFormrenderstate] = useState(sector.formrenderstate || "simpleform"); // todo simpleform
    ...
    if (content_type !== "application/json") {
      return setForm_Alerts((pr) => {
        return [
          ...pr,
          {
            severity: "danger", type: "simpleform",
      ...
      if(status === 200 || status === 201) {
        ...
        return setForm_Alerts((pr) => {
          return [
            ...pr,
            {
              severity: "success", type: "simpleform",
              title: `OK ${status}`,
    ...
    <div className="w-100" style={{position: "relative"}}>
      ...
      <div className="w-100 d-flex j-cont-between">
        <GenericForm 
          ...
          form_alerts={form_alerts.filter(f => f.type === "simpleform")}
