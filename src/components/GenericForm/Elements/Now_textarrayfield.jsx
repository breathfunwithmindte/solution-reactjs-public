...
const Now_textarrayfield = (input_props) => {
  ...
  return (
    <div 
      ...
      <Form.Control
        ...
        id={`${idname}`}
        value={
          current_value instanceof Array === true ? 
          current_value.join(current.as === "textarea" ? "\n" : ",") : ""
        }
        onChange={(e) => {
          ...
          input_props.onchange(
            e.target.type, e.target.name, 
            value.split(current.as === "textarea" ? "\n" : ",")
            );
        }}
        name={current.name}
        size={current.size}
...
export default Now_textarrayfield;